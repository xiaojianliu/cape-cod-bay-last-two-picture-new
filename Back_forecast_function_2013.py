...
class get_fvcom():
    ...
    def get_url(self, starttime, endtime):
        ...
        if self.modelname == "GOM3":
            ...
            """
            timeurl='''http://www.smast.umassd.edu:8080/thredds/dodsC/models/fvcom/NECOFS/Archive/Seaplan_33_Hindcast_v1/gom3_201211.nc?time[0:1:720]'''
            url='''http://www.smast.umassd.edu:8080/thredds/dodsC/models/fvcom/NECOFS/Archive/Seaplan_33_Hindcast_v1/gom3_201211.nc?lon[0:1:48450],lat[0:1:48450],lonc[0:1:90414],latc[0:1:90414],siglay[0:1:44][0:1:48450],h[0:1:48450],nv[0:1:2][0:1:90414],time[{0}:1:{1}],Times[{0}:1:{1}],zeta[{0}:1:{1}][0:1:48450],nbe[0:1:2][0:1:90414],u[{0}:1:{1}][0:1:44][0:1:90414],v[{0}:1:{1}][0:1:44][0:1:90414]'''
            """
            timeurl='gom3_201311.nc'
            #timeurl='''http://www.smast.umassd.edu:8080/thredds/dodsC/models/fvcom/NECOFS/Archive/Seaplan_33_Hindcast_v1/gom3_201211.nc?time[0:1:720]'''
            
            #timeurl = """http://www.smast.umassd.edu:8080/thredds/dodsC/fvcom/hindcasts/30yr_gom3?time[0:1:333551]"""#[0:1:316008]"""
            ...
            self.Times2=netCDF4.Dataset(timeurl).variables['Times'][:]#np.load('Times.npy')
            try:
            ...
            c=mtime[index1]
            print 'c',c
            d=datetime(1858,11,17)+timedelta(hours=c*24)#datetime.strptime(c[:-7], '%Y-%m-%d'+'T'+'%H:%M:%S')
            ...
            index2 = index1 + self.hours
            c1=mtime[index2]
            d1=datetime(1858,11,17)+timedelta(hours=c1*24)#datetime.strptime(c[:-7], '%Y-%m-%d'+'T'+'%H:%M:%S')
            #print 'Times[index1]',type(d1)
            print 'Times[index2]',d1
            #url = url.format(index1, index2)
            
            Times = []
            for i in range(len(mtime)):
                #self.hours+1):
                Times.append(datetime(1858,11,17)+timedelta(hours=mtime[i]*24))
            #print 'Times', Times
            self.mTime1 = Times
            #self.url = url
            self.xx=index1
            self.index2xx=index2
            self.hours1=self.hours+index1
        #print url
            print 'self.hours1',self.hours1
            print 'self.hours',self.hours
            print 'self.hours1-self.hours',self.hours1-self.hours
        return index1,index2
           
    def get_data(self,index1,index2):
        '''
        ...
        self.lonc=netCDF4.Dataset(url).variables['lonc'][:]
        self.latc=netCDF4.Dataset(url).variables['latc'][:]
        #= self.data['lonc'][:], self.data['latc'][:]  #quantity:165095
        self.lons=netCDF4.Dataset(url).variables['lon'][:]
        self.lats=netCDF4.Dataset(url).variables['lat'][:]
        #= self.data['lon'][:], self.data['lat'][:]
        self.h=netCDF4.Dataset(url).variables['h'][:]
        #= self.data['h'][:]; 
        self.siglay=netCDF4.Dataset(url).variables['siglay'][:]
        #= self.data['siglay'][:]; #nv = self.data['nv'][:]
        self.u=netCDF4.Dataset(url).variables['u']
        #= self.data['u']; 
        self.v=netCDF4.Dataset(url).variables['v']
        
        self.temp=netCDF4.Dataset(url).variables['temp']
        #= self.data['v']; 
        self.zeta=netCDF4.Dataset(url).variables['zeta']
        
        #= self.data['zeta']
        
        nbe1=netCDF4.Dataset(url).variables['nbe'][0]
        
        #self.data['nbe'][0];
        nbe2=netCDF4.Dataset(url).variables['nbe'][1]
        #self.data['nbe'][1];
        nbe3=netCDF4.Dataset(url).variables['nbe'][2]
        #self.data['nbe'][2]
    ...
    def get_track(self,lon,lat,depth,starttime,wind,wind_get_type,w): #,b_index,nvdepth, 
        ...
        '''
        windspeed=[]
        modpts = dict(lon=[lon], lat=[lat], layer=[], time=[],deep=[depth],temp=[]) #model forecast points
        ...
        t = abs(self.hours)
        txxxx=abs(self.hours1)
        tjjjj=txxxx-t
        
        try:
            if self.modelname == "GOM3" or self.modelname == "30yr":
                lonp,latp = self.nearest_point(lon, lat, lonl, latl,0.05)#elements(blue)
                
                lonn,latn = self.nearest_point(lon,lat,lonk,latk,0.05)#nodes(red)
            ...
            modpts['time'].append(self.mTime1[txxxx])
            
            print 'XXXXXXXXXhhhhhhhhhhhh',self.mTime1[txxxx]
            #modpts['temp'].append(self.temp[self.index2xx][0][nodeindex])
            ...
            modpts['temp'].append(self.temp[txxxx,layer,nodeindex][0])
        ...
        t = abs(self.hours)
        txxxx=abs(self.hours1)
        tjjjj=abs(self.hours)-t
        '''
        
        
        print 'hour',t         
        for i in xrange(txxxx,tjjjj,-1):
            if self.m!=10000:
                if i==self.m:
                    continue
            '''
            ...
            #print 'XXXXXXXXXXXXXXXXXXXXXXXXX2'
            ...
            #print 'XXXXXXXXXXXXXXXXXXXXXXXXX3'
            #if i!=(t-1):                
            try:
                #print 12345
                if self.modelname == "GOM3" or self.modelname == "30yr":
                    lonp,latp = self.nearest_point(lon, lat, lonl, latl,0.05)
                    lonn,latn = self.nearest_point(lon,lat,lonk,latk,0.05)
                    #print 'XXXXXXXXXXXXXXXXXXXXXXXXX4'
                if self.modelname == "massbay":
                    lonp,latp = self.nearest_point(lon, lat, lonl, latl,0.06)
                    lonn,latn = self.nearest_point(lon,lat,lonk,latk,0.1)
                index1 = np.where(self.lonc==lonp)
                ...
                #print 'XXXXXXXXXXXXXXXXXXXXXXXXX5'
                pa = self.eline_path(lon,lat)#boundary
                #print 'XXXXXXXXXXXXXXXXXXXXXXXXX6'
                if self.modelname == "30yr":            
                    waterdepth = self.h[nodeindex]
                    #print 'XXXXXXXXXXXXXXXXXXXXXXXXX7'
                else:
                    waterdepth = self.h[nodeindex]+self.zeta[i-1,nodeindex]
                modpts['time'].append(self.mTime1[i-1])    
                print 'XXXXXXXXX',self.mTime1[i-1]
                if depth+sum_deep>=0:
                    break
                ...
                modpts['temp'].append(self.temp[i-1,layer,nodeindex][0])
                #print modpts
                #if waterdepth<(abs(depth)): 
...
class get_fvcom2():
    ...
    def get_data(self,index1,index2):
        ...
        self.siglay=netCDF4.Dataset(url).variables['siglay'][:]
        #= self.data['siglay'][:]; #nv = self.data['nv'][:]
        self.u=netCDF4.Dataset(url).variables['u']#[index1:index2+1][0]
        #= self.data['u']; 
        self.v=netCDF4.Dataset(url).variables['v']#[index1:index2+1][0]
        
        self.temp=netCDF4.Dataset(url).variables['temp']
        #= self.data['v']; 
        self.zeta=netCDF4.Dataset(url).variables['zeta']#[index1:index2+1][0]
    ...
    def get_track(self,lon,lat,depth,starttime,wind,wind_get_type,w): #,b_index,nvdepth, 
        ...
        try:
            ...
            if self.modelname == "30yr":
                ...
                waterdepth = self.h[nodeindex]+self.zeta[0,nodeindex]
            modpts['time'].append(self.mTime1[txxxx])
            print 'XXXXXXXXX',self.mTime1[txxxx]
            #modpts['temp'].append(self.temp[self.index2xx][0][nodeindex])
            depth_total = self.siglay[:,nodeindex]*waterdepth; #print 'Here one' 
            layer = np.argmin(abs(depth_total+depth)); #print 'layer',layer
            modpts['layer'].append(layer); 
