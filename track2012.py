...
from Back_forecast_function_2012 import get_fvcom,get_fvcom2
import sys
...
tg2014 = np.genfromtxt('turtle2012.csv',dtype=None,names=['id','day','time','lat','lon'],delimiter=',',skip_header=1)
'''
plt.figure()
...
for a in np.arange(len(day)):
    if day[a][-6:-3]=='Dec':
        #print 1
        endtimes.append(dt.datetime(2012,12,int(day[a][:-7]),int(time[a][:-3]),int(time[a][-2:])))
    if day[a][-6:-3]=='Nov':
        #print 1
        endtimes.append(dt.datetime(2012,11,int(day[a][:-7]),int(time[a][:-3]),int(time[a][-2:])))
'''
    if day[a][-6:-3]=='Jan':
        #print 1
...
for a in np.arange(len(endtimes)):
    ...
    print 'st',st
    print 'end',end
    if end<datetime(2012,12,1,0,0,0):
        get_obj =  get_fvcom(Model)
        index1xx,index2xx = get_obj.get_url(st,end)                
        b_points = get_obj.get_data(index1xx,index2xx)
        back=dict(lon=[],lat=[],time=[],deep=[],temp=[]) 
        if a==5 or a==15 or a==93 or a==115 or a==126 or a==136 or a==157 or a==171 or a==234 or a==243 or a==259 or a==282 or a==287 or a==306 or a==313 or a==316 :
        ...
        elif a==42 or a==46 or a==47 or a==98 or a==44 or a==68 or a==83 or a==120 or a==172 or a==263:
            continue
        ...
        get_obj =  get_fvcom2(Model)
        index1xx,index2xx = get_obj.get_url(datetime(2012, 12, 1, 0, 0),end) 
        print 'index1xx',index1xx               
        b_points = get_obj.get_data(index1xx,index2xx)
        back=dict(lon=[],lat=[],time=[],deep=[],temp=[]) 
        if a==5 or a==15 or a==93 or a==115 or a==126 or a==136 or a==157 or a==171 or a==234 or a==243 or a==259 or a==282 or a==287 or a==306 or a==313 or a==316 :
            ...
            try:
                back,windspeed= get_obj.get_track(float(lon[a]),float(lat[a]),-1,st,wind,wind_get_type,0)
                if (back['time'][-1]-datetime(2012, 12, 1, 0, 0)).seconds<=60:
                    get_obj =  get_fvcom(Model)
                    index1xxx,index2xxx = get_obj.get_url(st,datetime(2012, 11, 30, 23, 0))                
                    b_points = get_obj.get_data(index1xxx,index2xxx)
                    back1=dict(lon=[],lat=[],time=[],deep=[],temp=[]) 
                    back1,windspeed1= get_obj.get_track(back['lon'][-1],back['lat'][-1],-1,st,wind,wind_get_type,0)
                    for axx in np.arange(len(back1['lon'])):
                        back['lon'].append(back1['lon'][axx])
                        back['lat'].append(back1['lat'][axx])
                        back['time'].append(back1['time'][axx])
                        back['deep'].append(back1['deep'][axx])
                        back['temp'].append(back1['temp'][axx])
            except:
                continue
...
plt.legend(loc='best')
plt.title('60-day trajectory of turtles in 2012')
plt.savefig('2012X',dpi=800)
np.save('lon2012X',wei_lon)
np.save('lat2012X',wei_lat)
np.save('lonnnnn2012X',lonnnnn)#.append(back['lon'])
np.save('lattttt2012X',lattttt)#.append(back['lat'])
np.save('timedian2012X',timedian)
np.save('temp2012X',tempz)
#tempz.append(back['temp'])
"""
