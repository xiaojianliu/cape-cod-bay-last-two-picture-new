...
from Back_forecast_function_2013 import get_fvcom,get_fvcom2
import sys
...
for a in np.arange(len(endtimes)):
    ...
    print 'st',st
    print 'end',end
    if end<datetime(2013,12,1,0,0,0):
        ...
        index1xx,index2xx = get_obj.get_url(datetime(2013, 12, 1, 0, 0),end) 
        ...
        if a==53 or a==126 :
            ...
            try:
                back,windspeed= get_obj.get_track(float(lon[a]),float(lat[a]),-1,st,wind,wind_get_type,0)
                if (back['time'][-1]-datetime(2013, 12, 1, 0, 0)).seconds<=60:
                    get_obj =  get_fvcom(Model)
                    index1xxx,index2xxx = get_obj.get_url(st,datetime(2013, 11, 30, 23, 0))                
...
plt.savefig('2013X',dpi=800)
np.save('lon2013X',wei_lon)
np.save('lat2013X',wei_lat)
np.save('lonnnnn2013X',lonnnnn)#.append(back['lon'])
np.save('lattttt2013X',lattttt)#.append(back['lat'])
np.save('time2013X',timedian)
np.save('temp2013X',tempp)
